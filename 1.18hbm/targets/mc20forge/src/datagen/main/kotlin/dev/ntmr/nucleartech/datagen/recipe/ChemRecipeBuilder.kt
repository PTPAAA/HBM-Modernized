...
class ChemRecipeBuilder(private val duration: Int) : RecipeBuilder {
    ...
    override fun save(consumer: Consumer<FinishedRecipe>) {
        if (results.isNotEmpty()) {
            if (outputFluids.isNotEmpty()) save(consumer, ntm("${RecipeBuilder.getDefaultRecipeId(result).path}_and_${ForgeRegistries.FLUIDS.getKey(outputFluids.firstOrNull()?.fluid)?.path}_from_chemistry"))
            else save(consumer, ntm("${RecipeBuilder.getDefaultRecipeId(result).path}_from_chemistry"))
        } else save(consumer, ntm("${ForgeRegistries.FLUIDS.getKey(outputFluids.firstOrNull()?.fluid)?.path}_from_chemistry"))
    }
    ...
    override fun save(consumer: Consumer<FinishedRecipe>, recipeName: ResourceLocation) {
        ...
        consumer.accept(Result(recipeName, ingredients, inputFluid1 to inputFluid2, results, outputFluid1 to outputFluid2, duration, advancement, ResourceLocation(recipeName.namespace, "recipes/chemistry/${recipeName.path}")))
    }
    ...
    private class Result(
        ...
        override fun getType() = NTechRecipeSerializers.CHEM.get()
