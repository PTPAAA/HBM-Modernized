...
import dev.ntmr.nucleartech.*
import dev.ntmr.nucleartech.datagen.recipe.*
...
import dev.ntmr.nucleartech.datagen.recipe.AnvilConstructingRecipeBuilder as AnvilRecipeBuilder
import dev.ntmr.nucleartech.content.recipe.anvil.AnvilConstructingRecipe.OverlayType as AnvilRecipeType
...
class NuclearRecipeProvider(generator: DataGenerator) : RecipeProvider(generator) {
    ...
    override fun buildCraftingRecipes(consumer: Consumer<FinishedRecipe>) {
        ...
        SpecialRecipeBuilder.special(NTechRecipeSerializers.SMITHING_RENAMING.get()).save(consumer, ntm("anvil_smithing_renaming").toString())
...
//        ShapedRecipeBuilder.shaped(NTechBlockItems.blastFurnace.get()).define('T', NTechTags.Items.INGOTS_TUNGSTEN).define('C', NTechItems.copperPanel.get()).define('H', Items.HOPPER).define('F', Items.FURNACE).pattern("T T").pattern("CHC").pattern("TFT").group(NTechBlockItems.blastFurnace.id.path).unlockedBy("has_${NTechItems.tungstenIngot.get()}", has(NTechTags.Items.INGOTS_TUNGSTEN)).save(consumer, NTechBlockItems.blastFurnace.id)
    ...
    private enum class SubDirectories(type: RecipeType<*>) {
        ...
        ASSEMBLING(NTechRecipeTypes.ASSEMBLY),
        BLASTING(NTechRecipeTypes.BLASTING),
        CENTRIFUGING(NTechRecipeTypes.CENTRIFUGE),
        CHEMISTRY(NTechRecipeTypes.CHEM),
        CONSTRUCTING(NTechRecipeTypes.CONSTRUCTING),
        PRESSING(NTechRecipeTypes.PRESSING),
        SHREDDING(NTechRecipeTypes.SHREDDING),
        SMITHING(NTechRecipeTypes.SMITHING),
        ;
