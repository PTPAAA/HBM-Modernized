...
class AnvilConstructingRecipeBuilder(val tierLower: Int = 1, val tierUpper: Int = -1) : RecipeBuilder {
    ...
    fun requires(vararg item: ItemLike) = requires(item.map { StackedIngredient.of(1, it) }).also { if (advancement.criteria.isEmpty()) unlockedBy("has_${ForgeRegistries.ITEMS.getKey(item.first().asItem())!!.path}", has(item.first())) }
    ...
    fun requires(vararg requirements: Pair<Int, ItemLike>) = requires(requirements.map { (amount, item) -> StackedIngredient.of(amount, item) }).also { if (advancement.criteria.isEmpty()) unlockedBy("has_${ForgeRegistries.ITEMS.getKey(requirements.first().second.asItem())!!.path}", has(requirements.first().second)) }
    @JvmName("requiresStackPair")
    fun requires(vararg requirements: Pair<Int, ItemStack>) = requires(requirements.map { (amount, stack) -> StackedIngredient.of(amount, stack) }).also { if (advancement.criteria.isEmpty()) unlockedBy("has_${ForgeRegistries.ITEMS.getKey(requirements.first().second.item)!!.path}", has(requirements.first().second.item)) }
    ...
    private fun inventoryTrigger(vararg requirements: ItemPredicate) = InventoryChangeTrigger.TriggerInstance(ContextAwarePredicate.ANY, MinMaxBounds.Ints.ANY, MinMaxBounds.Ints.ANY, MinMaxBounds.Ints.ANY, requirements)
    ...
    override fun save(consumer: Consumer<FinishedRecipe>, recipeName: ResourceLocation) {
        ...
        consumer.accept(Result(recipeName, tierLower, tierUpper, ingredients, output, overlay, advancement, ResourceLocation(recipeName.namespace, "recipes/anvil_constructing/${recipeName.path}")))
    }
    ...
    private class Result(
        ...
        override fun getType() = NTechRecipeSerializers.CONSTRUCTING.get()
