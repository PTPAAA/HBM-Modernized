...
class BatteryRecipeBuilder(private val result: Item, val count: Int) : RecipeBuilder {
    ...
    override fun save(consumer: Consumer<FinishedRecipe>, name: ResourceLocation) {
        ...
        consumer.accept(Result(name, result, count, group, rows, key, advancement, ResourceLocation(name.namespace, "recipes/batteries/" + name.path)))
    }
    ...
    class Result(
        ...
        override fun serializeRecipeData(p_218610_1_: JsonObject) {
            ...
            jsonobject1.addProperty(
                ...
                (ForgeRegistries.ITEMS.getKey(result)
                    ?: throw IllegalStateException("No item ${result} registered")).toString()
            )
            if (this.count > 1) {
        ...
        override fun getType() = NTechRecipeSerializers.BATTERY.get()
    ...
    companion object {
        ...
        private fun getItemName(item: ItemLike) = ForgeRegistries.ITEMS.getKey(item.asItem())!!.path
    }
}
