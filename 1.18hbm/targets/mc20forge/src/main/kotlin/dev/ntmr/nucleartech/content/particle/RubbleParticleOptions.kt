...
class RubbleParticleOptions(val block: Block) : ParticleOptions {
    ...
    override fun getType() = NTechParticles.RUBBLE.get()

    override fun writeToNetwork(packet: FriendlyByteBuf) {
        packet.writeRegistryId(ForgeRegistries.BLOCKS, block)
    }
    ...
    companion object {
        ...
        val DESERIALIZER = object : ParticleOptions.Deserializer<RubbleParticleOptions> {
            ...
            override fun fromNetwork(type: ParticleType<RubbleParticleOptions>, packet: FriendlyByteBuf) = RubbleParticleOptions(packet.readRegistryIdSafe(Block::class.java))
        }

        @JvmStatic
        val CODEC: Codec<RubbleParticleOptions> = RecordCodecBuilder.create { builder ->
            builder.group(ResourceLocation.CODEC.fieldOf("block").forGetter { ForgeRegistries.BLOCKS.getKey(it.block) }).apply(builder, ::RubbleParticleOptions)
        }
    }
}
