...
import dev.ntmr.nucleartech.system.io.energy.Cable
...
class CableBlockEntity(pos: BlockPos, state: BlockState) : AbstractTransmitterBlockEntity(NTechBlockEntities.cableBlockEntityType.get(), pos, state) {
    ...
    override fun <T : Any> getCapability(cap: Capability<T>, side: Direction?): LazyOptional<T> {
        if (!remove && cap == ForgeCapabilities.ENERGY) return capability.cast()
