...
import dev.ntmr.nucleartech.content.item.BatteryItem
...
class BatteryRecipe(private val delegate: ShapedRecipe) : CraftingRecipe, IShapedRecipe<CraftingContainer> by delegate { // i got frustrated on having to use access transformers and everything. then i remembered kotlin has delegation...
    ...
    override fun assemble(inventory: CraftingContainer, registryAccess: RegistryAccess): ItemStack {
        val resultItem = delegate.getResultItem(registryAccess)
    ...
    override fun getSerializer() = NTechRecipeSerializers.BATTERY.get()
    ...
    class Serializer : RecipeSerializer<BatteryRecipe> {
