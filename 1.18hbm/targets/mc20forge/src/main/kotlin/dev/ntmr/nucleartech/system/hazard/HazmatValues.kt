...
package dev.ntmr.nucleartech.system.hazard

import dev.ntmr.nucleartech.SpecialUsers
import dev.ntmr.nucleartech.api.hazard.radiation.HazmatRegistry
import dev.ntmr.nucleartech.content.NTechAttributes
...
object HazmatValues : HazmatRegistry {
    ...
    private val repairItemResistances = mutableMapOf<Item, Float>()
    private val specialResistances = mutableMapOf<Item, Float>()
    ...
    fun addItemStackAttributes(event: ItemAttributeModifierEvent) = with(event) {
        ...
        if (item is ArmorItem) {
            ...
            var resistance = specialResistances[item] ?: materialResistances[item.material.name] ?: repairItemResistances.firstNotNullOfOrNull { (repairItem, value) -> if (item.material.repairIngredient.test(repairItem.defaultInstance)) value else null } ?: return
            ...
            event.addModifier(NTechAttributes.RADIATION_RESISTANCE.get(), AttributeModifier(SLOT_UUIDS[slot.index], "Armor radiation resistance", resistance.toDouble(), AttributeModifier.Operation.ADDITION))
        }
    }

    fun getPlayerResistance(player: Player): Float {
        var resistance = 0F
        resistance += player.getAttributeValue(NTechAttributes.RADIATION_RESISTANCE.get()).toFloat()
        if (player.stringUUID == SpecialUsers.Pu_238) resistance += .4F
        return resistance
    }
    ...
    override fun registerArmorRepairItem(ingredient: Supplier<out Item>, fullSetProtection: Float): Boolean {
        val item = ingredient.get()
        if (repairItemResistances.contains(item)) return false
        repairItemResistances += item to fullSetProtection
        return true
    }

    override fun registerItem(item: Supplier<out Item>, protection: Float): Boolean {
        val i = item.get()
        if (specialResistances.contains(i)) return false
        specialResistances += i to protection
        return true
    }
}
