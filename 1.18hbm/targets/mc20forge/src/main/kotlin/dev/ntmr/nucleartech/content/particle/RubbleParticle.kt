...
import dev.ntmr.nucleartech.extensions.surroundPath
...
class RubbleParticle(level: ClientLevel, x: Double, y: Double, z: Double, xd: Double, yd: Double, zd: Double, private val texture: ResourceLocation) : Particle(level, x, y, z) {
    ...
    private val deltaRot = Vector3d(random.nextDouble() * 10.0 - 5.0, random.nextDouble() * 10.0 - 5.0, random.nextDouble() * 10.0 - 5.0)
    private val sinkSpeed = random.nextDouble() * 0.0015 + 0.0005
    ...
    override fun tick() {
        ...
        if (!wasStopped && stoppedByCollision) {
            level.playLocalSound(x, y, z, NTechSounds.debris.get(), SoundSource.BLOCKS, 1.5F, 1F, false)
        }
    }
    ...
    override fun render(consumer: VertexConsumer, camera: Camera, partials: Float) {
        ...
        stack.mulPose(com.mojang.math.Axis.XP.rotationDegrees(rotationX))
        stack.mulPose(com.mojang.math.Axis.YP.rotationDegrees(rotationY))
        stack.mulPose(com.mojang.math.Axis.ZP.rotationDegrees(rotationZ))
    ...
    class Provider : ParticleProvider<RubbleParticleOptions> {
        override fun createParticle(options: RubbleParticleOptions, level: ClientLevel, x: Double, y: Double, z: Double, xd: Double, yd: Double, zd: Double) =
            RubbleParticle(level, x, y, z, xd, yd, zd, Minecraft.getInstance().blockRenderer.blockModelShaper.getTexture(options.block.defaultBlockState(), level, BlockPos.containing(x, y, z)).contents().name().surroundPath("textures/", ".png"))
    }
}
