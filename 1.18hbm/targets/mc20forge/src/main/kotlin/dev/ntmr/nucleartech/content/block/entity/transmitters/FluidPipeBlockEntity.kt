...
import dev.ntmr.nucleartech.system.io.fluid.Pipe
...
class FluidPipeBlockEntity(pos: BlockPos, state: BlockState) : AbstractTransmitterBlockEntity(NTechBlockEntities.fluidPipeBlockEntityType.get(), pos, state) {
    ...
    override fun getContinuousUpdateTag(): CompoundTag = super.getContinuousUpdateTag().apply {
        putString("Fluid", ForgeRegistries.FLUIDS.getKey(fluid)?.toString() ?: "minecraft:empty")
    }
    ...
    override fun saveAdditional(tag: CompoundTag) {
        ...
        val fluidLocation = ForgeRegistries.FLUIDS.getKey(fluid)
    ...
    override fun <T : Any> getCapability(cap: Capability<T>, side: Direction?): LazyOptional<T> {
        if (!remove && cap == ForgeCapabilities.FLUID_HANDLER) return capability.cast()
