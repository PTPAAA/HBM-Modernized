...
import dev.ntmr.nucleartech.content.item.transferItemsBetweenItemHandlers
import dev.ntmr.nucleartech.extensions.component1
import dev.ntmr.nucleartech.extensions.component2
import dev.ntmr.nucleartech.extensions.component3
import dev.ntmr.nucleartech.extensions.ifPresentInline
...
interface IODelegatedBlockEntity {
    ...
    companion object {
        ...
        val DEFAULT_ITEM_ACTION = IOConfiguration.IOAction { self: BlockEntity, other: BlockEntity, side: Direction, mode: IOConfiguration.Mode, selfCache: CapabilityCache, otherCache: CapabilityCache  ->
            selfCache.getOrAddToCache(ForgeCapabilities.ITEM_HANDLER, side, self::getCapability).ifPresentInline { selfHandler ->
                otherCache.getOrAddToCache(ForgeCapabilities.ITEM_HANDLER, side.opposite, other::getCapability).ifPresentInline { otherHandler ->
                    if (mode.isInput()) {
        ...
        val DEFAULT_ENERGY_ACTION = IOConfiguration.IOAction { self: BlockEntity, other: BlockEntity, side: Direction, mode: IOConfiguration.Mode, selfCache: CapabilityCache, otherCache: CapabilityCache ->
            selfCache.getOrAddToCache(ForgeCapabilities.ENERGY, side, self::getCapability).ifPresentInline { selfHandler ->
                otherCache.getOrAddToCache(ForgeCapabilities.ENERGY, side.opposite, other::getCapability).ifPresentInline { otherHandler ->
                    if (mode.isInput()) {
        ...
        val DEFAULT_FLUID_ACTION = IOConfiguration.IOAction { self: BlockEntity, other: BlockEntity, side: Direction, mode: IOConfiguration.Mode, selfCache: CapabilityCache, otherCache: CapabilityCache ->
            selfCache.getOrAddToCache(ForgeCapabilities.FLUID_HANDLER, side, self::getCapability).ifPresentInline { selfHandler ->
                otherCache.getOrAddToCache(ForgeCapabilities.FLUID_HANDLER, side.opposite, other::getCapability).ifPresentInline { otherHandler ->
                    // TODO
                }
            }
        }
