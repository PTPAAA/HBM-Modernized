...
class AnvilMenu(windowID: Int, val playerInventory: Inventory, val access: ContainerLevelAccess, val tier: Int) : AbstractContainerMenu(NTechMenus.anvilMenu.get(), windowID) {
    ...
    private val recipes = level.recipeManager.getAllRecipesFor(NTechRecipeTypes.SMITHING)
    ...
    private fun onTake(player: Player, stack: ItemStack) {
        ...
        val ingredients = buildList {
            if (!inputSlots.getItem(0).isEmpty) add(inputSlots.getItem(0))
            if (!inputSlots.getItem(1).isEmpty) add(inputSlots.getItem(1))
        }
        resultSlot.awardUsedRecipes(player, ingredients)
    ...
    private fun createResult() {
        val matchingRecipes = level.recipeManager.getRecipesFor(NTechRecipeTypes.SMITHING, inputSlots, level).filter { it.tier <= tier }
        if (matchingRecipes.isEmpty()) resultSlot.setItem(0, ItemStack.EMPTY)
        else {
            selectedRecipe = matchingRecipes.first()
            val result = selectedRecipe!!.assemble(inputSlots, level.registryAccess())
