...
import dev.ntmr.nucleartech.content.item.PolaroidItem
...
object NTechCapes {
    ...
    private class NTechCapeLayer(parent: RenderLayerParent<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>>) : RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>>(parent) {
        override fun render(matrix: PoseStack, buffers: MultiBufferSource, light: Int, player: AbstractClientPlayer, p_117353_: Float, p_117354_: Float, partials: Float, p_117356_: Float, p_117357_: Float, p_117358_: Float) {
            ...
            matrix.mulPose(Axis.XP.rotationDegrees(6F + xRot / 2F + yRot))
            matrix.mulPose(Axis.ZP.rotationDegrees(zRot / 2F))
            matrix.mulPose(Axis.YP.rotationDegrees(180F - zRot / 2F))
    ...
    private val playerCapes = with(SpecialUsers) {
        mapOf(
    ...
    // TODO: TextureAtlasHolder API changed in 1.20, getResourcesToLoad removed
    class CapeTextureManager(textureManager: TextureManager) : TextureAtlasHolder(textureManager, CAPE_TEXTURE_ATLAS_LOCATION, ResourceLocation("cape")) {
        // getResourcesToLoad() removed in 1.20 - textures are loaded via data-driven atlases now
