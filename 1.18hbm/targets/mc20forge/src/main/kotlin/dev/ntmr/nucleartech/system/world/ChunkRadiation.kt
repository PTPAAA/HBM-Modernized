...
object ChunkRadiation : ChunkRadiationHandler {
    ...
    fun onWorldLoad(event: LevelEvent.Load) {
        if (!event.level.isClientSide) perWorldRadiation[event.level] = mutableMapOf()
    }

    @SubscribeEvent @JvmStatic
    fun onWorldUnload(event: LevelEvent.Unload) {
        if (!event.level.isClientSide) perWorldRadiation.remove(event.level)
    }

    private const val RADIATION_KEY = MODID + "_radiation"

    @SubscribeEvent @JvmStatic
    fun onChunkLoad(event: ChunkDataEvent.Load) {
        if (event.level == null || event.level.isClientSide) return
        val worldRadiation = perWorldRadiation[event.level] ?: return
    ...
    fun onChunkUnload(event: ChunkEvent.Unload) {
        if (event.level.isClientSide) return
        perWorldRadiation[event.level]?.remove(event.chunk.pos)
    }
