...
import dev.ntmr.nucleartech.client.rendering.renderModelAlpha
...
class ChemPlantRenderer(@Suppress("UNUSED_PARAMETER") context: BlockEntityRendererProvider.Context) : BlockEntityRenderer<ChemPlantBlockEntity> {
    ...
    override fun render(chemPlant: ChemPlantBlockEntity, partials: Float, matrix: PoseStack, buffers: MultiBufferSource, light: Int, overlay: Int) {
        ...
        matrix.mulPose(Axis.YN.rotationDegrees(chemPlant.blockState.getValue(HorizontalDirectionalBlock.FACING).toYRot()))
        matrix.translate(-.5, .0, -.5)
        ...
        if (!fluid1.isEmpty && chemPlant.canProgress) matrix.mulPose(Axis.YP.rotationDegrees(-spinnerRotation))
        else matrix.mulPose(Axis.YP.rotationDegrees(-45F))
        ...
        matrix.pushPose()
        matrix.translate(.625, .0, .625)
        if (!fluid2.isEmpty && chemPlant.canProgress) matrix.mulPose(Axis.YP.rotationDegrees(-spinnerRotation))
        else matrix.mulPose(Axis.YP.rotationDegrees(45F))
        spinner.render(matrix, buffers, RenderType::entityCutoutNoCull, light, overlay, partials, MultipartTransforms.EMPTY)
        matrix.popPose()
        ...
        if (!fluid1.isEmpty) {
            ...
            val color = net.minecraftforge.client.extensions.common.IClientFluidTypeExtensions.of(fluid1.fluid).tintColor
            ...
            for (i in 0 until count) {
                if (i == count - 1) modelRenderer.renderModelAlpha(matrix.last(), renderBuffer, null, fluidCap, red, green, blue, alpha, light, overlay, ModelData.EMPTY)
                else modelRenderer.renderModelAlpha(matrix.last(), renderBuffer, null, fluidPart, red, green, blue, alpha, light, overlay, ModelData.EMPTY)
        ...
        if (!fluid2.isEmpty) {
            ...
            val color = net.minecraftforge.client.extensions.common.IClientFluidTypeExtensions.of(fluid2.fluid).tintColor
            val red = (color shr 16 and 0xFF) / 255F
            val green = (color shr 8 and 0xFF) / 255F
            val blue = (color and 0xFF) / 255F
            val alpha = (color shr 24 and 0xFF) / 255F
    ...
    private fun getTextureForFluid(chemPlant: ChemPlantBlockEntity, fluid: FluidStack) =
        if (chemPlant.canProgress) net.minecraftforge.client.extensions.common.IClientFluidTypeExtensions.of(fluid.fluid).getFlowingTexture(fluid)
        else net.minecraftforge.client.extensions.common.IClientFluidTypeExtensions.of(fluid.fluid).getStillTexture(fluid)
