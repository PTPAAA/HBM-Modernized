...
import dev.ntmr.nucleartech.content.item.ShredderBladeItem
...
class ShredderBlockEntity(pos: BlockPos, state: BlockState) : ProgressingMachineBlockEntity(NTechBlockEntities.shredderBlockEntityType.get(), pos, state), RecipeHolder, ExperienceRecipeResultBlockEntity {
    var energy: Int
    ...
    override fun isItemValid(slot: Int, stack: ItemStack): Boolean = when (slot) {
        9 -> stack.getCapability(ForgeCapabilities.ENERGY).isPresent
    ...
    private fun process(simulate: Boolean): Boolean {
        ...
        for (item in input.getItems(false)) { // TODO still a bit of a mess, but not as bad
            ...
            val recipe = recipeManager.getRecipeFor(NTechRecipeTypes.SHREDDING, container, level).getOrNull()
            if (recipe == null) {
