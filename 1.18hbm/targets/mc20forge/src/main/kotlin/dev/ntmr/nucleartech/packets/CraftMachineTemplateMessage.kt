...
import dev.ntmr.nucleartech.content.item.giveItemToInventory
    ...
    override fun handle(context: Supplier<NetworkEvent.Context>) {
        if (context.get().direction.receptionSide.isServer)
            context.get().enqueueWork {
                ...
                if (!sender.inventory.getSelected().`is`(NTechItems.machineTemplateFolder.get().defaultInstance.item) &&
                    !sender.inventory.offhand.first().`is`(NTechItems.machineTemplateFolder.get().defaultInstance.item)) {
