...
import dev.ntmr.nucleartech.math.*
import dev.ntmr.nucleartech.packets.ExplosionVNTMessage
...
class ExplosionVNT(
    ...
    val compat = object : Explosion(level, exploder, null, null, pos.x, pos.y, pos.z, size, false, interaction) {
    ...
    fun interface BlockAllocator {
        ...
        class Default(private val resolution: Int = 16) : BlockAllocator {
            override fun allocate(explosion: ExplosionVNT, level: Level, pos: Vec3, size: Float): Set<BlockPos> {
                ...
                for (i in 0 until resolution) for (j in 0 until resolution) for (k in 0 until resolution) if (i == 0 || j == 0 || k == 0 || i == resolution - 1 || j == resolution - 1 || k == resolution - 1) {
                    ...
                    while (force > 0F) {
                        val blockPos = BlockPos.containing(px, py, pz)
    ...
    fun interface BlockProcessor {
        ...
        class Default(private val dropChanceMutator: DropChanceMutator? = null, private val fortuneMutator: FortuneMutator? = null, private val blockMutator: BlockMutator? = null) : BlockProcessor {
            override fun process(explosion: ExplosionVNT, level: Level, pos: Vec3, affectedBlocks: Set<BlockPos>) {
                if (explosion.interaction == BlockInteraction.KEEP) return

                val resources = ObjectArrayList<Pair<ItemStack, BlockPos>>()
                val shuffledPositions = affectedBlocks.toList().shuffled(java.util.Random(level.random.nextLong()))
                    ...
                    if (!blockState.isAir) {
                        ...
                        if (dropChanceMutation != 0F && level is ServerLevel && blockState.canDropFromExplosion(level, blockPos, explosion.compat)) {
                            ...
                            if (explosion.interaction == BlockInteraction.DESTROY) {
                                ...
                                lootParamsBuilder.withParameter(LootContextParams.EXPLOSION_RADIUS, virtualSize)
                            }

                            dropLoop@for (drop in blockState.getDrops(lootParamsBuilder)) {
        ...
        class Default : ClientSyncer {
            override fun sync(explosion: ExplosionVNT, level: ServerLevel, pos: Vec3, size: Float, affectedBlocks: List<BlockPos>, affectedPlayers: Map<Player, Vec3>) {
                for (player in level.players()) if (player.distanceToSqr(pos) < 4096.0)
                    NuclearPacketHandler.send(PacketDistributor.PLAYER.with { player }, ExplosionVNTMessage(pos, size, if (explosion.interaction == BlockInteraction.KEEP) emptyList() else affectedBlocks.toList(), affectedPlayers.getOrDefault(player, Vec3.ZERO)))
            }
        }
    }
