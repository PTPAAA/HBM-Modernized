...
open class RBMKBurningDebrisBlock(properties: Properties) : RBMKDebrisBlock(properties) {
    override fun tick(state: BlockState, level: ServerLevel, pos: BlockPos, random: net.minecraft.util.RandomSource) {
    ...
    private fun spawnFlame(level: ServerLevel, pos: BlockPos, random: net.minecraft.util.RandomSource) {
        level.sendParticles(
            NTechParticles.RBMK_FIRE.get(), true,
    ...
    protected open fun getNextTickSchedule(random: net.minecraft.util.RandomSource) = 100 + random.nextInt(20)
}
