...
object EventSubscribers {
    ...
    @SubscribeEvent @JvmStatic
    fun tickWorld(event: TickEvent.LevelTickEvent) {
    ...
    fun onLivingUpdate(event: LivingEvent.LivingTickEvent) {
        event.entity.armorSlots.forEach { val item = it.item; if (item is TickingArmor) item.tickArmor(event, it) }
        EntityContaminationEffects.update(event.entity)
        HazardSystem.applyHazardsInInventory(event.entity)
    }

    @SubscribeEvent @JvmStatic
    fun onLivingSpawn(event: MobSpawnEvent.FinalizeSpawn) {
        if (EntityContaminationEffects.isRadiationImmune(event.entity) || event.spawnType != MobSpawnType.NATURAL) return
        val level = event.level
        val pos = BlockPos.containing(event.x, event.y, event.z)
        if (ChunkRadiation.getRadiation(level, pos) > 2F) event.setSpawnCancelled(true)
    }

    @SubscribeEvent @JvmStatic
    fun onLivingAttack(event: LivingAttackEvent) {
        event.entity.armorSlots.forEach { val item = it.item; if (item is AttackHandler) item.handleAttack(event, it) }
    }

    @SubscribeEvent @JvmStatic
    fun onLivingHurt(event: LivingHurtEvent) {
        event.entity.armorSlots.forEach { val item = it.item; if (item is DamageHandler) item.handleDamage(event, it) }
    }

    @SubscribeEvent @JvmStatic
    fun onLivingFall(event: LivingFallEvent) {
        event.entity.armorSlots.forEach { val item = it.item; if (item is FallHandler) item.handleFall(event, it) }
    }
