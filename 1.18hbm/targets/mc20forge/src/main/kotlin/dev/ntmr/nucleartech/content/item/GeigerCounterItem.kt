...
import dev.ntmr.nucleartech.extensions.append
...
class GeigerCounterItem(properties: Properties) : Item(properties) {
    override fun use(world: Level, player: Player, hand: InteractionHand): InteractionResultHolder<ItemStack> {
        player.playSound(NTechSounds.randomBoop.get(), 1F, 1F)
        ...
        if (world.gameTime % 5L == 0L) if (radPerSecond > 0) {
            ...
            if ((likelihood == 1 || world.random.nextInt(likelihood) == 0)) {
                val sound = when (Random.nextInt(intensity.countTrailingZeroBits(), intensity.countOneBits() + intensity.countTrailingZeroBits()) + 1) {
                    1 -> NTechSounds.geiger1
                    2 -> NTechSounds.geiger2
                    3 -> NTechSounds.geiger3
                    4 -> NTechSounds.geiger4
                    5 -> NTechSounds.geiger5
                    6 -> NTechSounds.geiger6
                    else -> NTechSounds.geiger1
                }
            ...
            world.playSound(null, entity,
                if (world.random.nextInt(2) == 0) NTechSounds.geiger2.get() else NTechSounds.geiger1.get(),
