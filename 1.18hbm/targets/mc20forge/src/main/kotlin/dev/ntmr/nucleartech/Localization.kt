...
object LangKeys {
    ...
    val DEVICE_POLLUTION_DETECTOR_HEAVY_METAL = of(CAT_DEVICE, "pollution_detector.heavy_metal") // 1 float
    val DEVICE_POLLUTION_DETECTOR_POISON = of(CAT_DEVICE, "pollution_detector.poison") // 1 float
    val DEVICE_POLLUTION_DETECTOR_SOOT = of(CAT_DEVICE, "pollution_detector.soot") // 1 float
    ...
    val FLUID_TRAIT_POLLUTING_BURNED = of(CAT_FLUID_TRAIT, "polluting.burned")
    val FLUID_TRAIT_POLLUTING_SPILLED = of(CAT_FLUID_TRAIT, "polluting.spilled")
    val FLUID_TRAIT_POLLUTING_TYPE_HEAVY_METAL = of(CAT_FLUID_TRAIT, "polluting.type.heavy_metal") // 1 float, 1 string
    val FLUID_TRAIT_POLLUTING_TYPE_POISON = of(CAT_FLUID_TRAIT, "polluting.type.poison") // 1 float, 1 string
    val FLUID_TRAIT_POLLUTING_TYPE_SOOT = of(CAT_FLUID_TRAIT, "polluting.type.soot") // 1 float, 1 string
    ...
    @Suppress("NOTHING_TO_INLINE")
    private inline fun of(category: String, path: String, namespace: String = MODID) = TranslationKey(category, path, namespace)
}

@JvmInline
value class TranslationKey(val key: String) : Supplier<MutableComponent> {
    constructor(category: String, path: String, namespace: String = MODID) : this("$category.$namespace.$path")
...
fun ntmTranslationString(key: String) = "$MODID.$key"

fun Float.roundToPoints(decimalPoints: Int): Float = times(10F.pow(decimalPoints)).toInt().div((10F.pow(decimalPoints)))
fun Double.roundToPoints(decimalPoints: Int): Double = times(10.0.pow(decimalPoints)).toInt().div((10.0.pow(decimalPoints)))
