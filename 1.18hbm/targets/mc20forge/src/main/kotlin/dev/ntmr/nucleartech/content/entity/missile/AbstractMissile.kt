...
import dev.ntmr.nucleartech.content.particle.ContrailParticleOptions
...
import dev.ntmr.nucleartech.system.world.ChunkLoader
...
abstract class AbstractMissile : Entity, ChunkLoader {
    ...
    constructor(entityType: EntityType<out AbstractMissile>, level: Level, startPos: BlockPos, targetPos: BlockPos) : super(entityType, level) {
        ...
        if (!level().isClientSide) (level() as ServerLevel).sendParticles(ContrailParticleOptions(Vector3f(), .75F), true, x, y - .5, z, 40, .9, .1, .9, 5.0)
    }

    open val renderTexture: ResourceLocation get() = missileTexture(ForgeRegistries.ENTITY_TYPES.getKey(type)!!.path)
...
//        unRide()
    ...
    override fun tick() {
        ...
        for (i in 0 until velocity) {
            ...
            if (!level().isClientSide) (level() as ServerLevel).sendParticles(ContrailParticleOptions(Vector3f(), .4F), true, x, y, z, 6, 0.0, 0.0, 0.0, 0.0)
...
//                    unRide()
    ...
    private fun forceChunk(chunkPos: ChunkPos) = if (chunkPos !in forcedChunks) (ForgeChunkManager.forceChunk(level() as ServerLevel, MODID, this, chunkPos.x, chunkPos.z, true, false) && ForgeChunkManager.forceChunk(level() as ServerLevel, MODID, this, chunkPos.x, chunkPos.z, true, true)).also { forcedChunks += chunkPos } else false
    private fun unForceChunk(chunkPos: ChunkPos) = if (chunkPos in forcedChunks) (ForgeChunkManager.forceChunk(level() as ServerLevel, MODID, this, chunkPos.x, chunkPos.z, false, false) && ForgeChunkManager.forceChunk(level() as ServerLevel, MODID, this, chunkPos.x, chunkPos.z, false, true)).also { forcedChunks -= chunkPos } else false
...
//    override fun getPassengersRidingOffset() = 1.5
    ...
    override fun teleportTo(x: Double, y: Double, z: Double) {
        ...
        if (!level().isClientSide) {
            startPos = BlockPos(x.toInt(), y.toInt(), z.toInt())
    ...
    override fun getAddEntityPacket(): Packet<net.minecraft.network.protocol.game.ClientGamePacketListener> = NetworkHooks.getEntitySpawningPacket(this)
