...
class FluidIdentifierItem(properties: Properties) : TooltipItem(properties) {
    override fun getName(stack: ItemStack): Component = Component.translatable(getDescriptionId(stack), getFluid(stack).let { if (it.isSame(Fluids.EMPTY)) Component.literal("N/A") else Component.translatable(it.fluidType.descriptionId) })
    ...
    companion object {
        ...
        fun getAllFluidIdentifiers(): List<ItemStack> =
            ForgeRegistries.FLUIDS
                .filterNot { ForgeRegistries.FLUIDS.getKey(it)!!.path.contains("flowing") }
                .map { ItemStack(NTechItems.fluidIdentifier.get()).apply { orCreateTag.putString("FluidId", ForgeRegistries.FLUIDS.getKey(it)!!.toString()) } }
    }
}
