...
import dev.ntmr.nucleartech.system.capability.contamination.addEffect
...
class EmptyIVBagItem(properties: Properties) : IVBagItem(properties) {
    override val bagUseSound = NTechSounds.inject
    ...
    override fun doBagUseAction(level: Level, player: Player, originalStack: ItemStack, hand: InteractionHand, originalStackEmptied: Boolean): InteractionResultHolder<ItemStack> {
        if (!player.abilities.instabuild) {
            if (player.health > 5F) {
                // TODO: Fix DamageSources.extractBlood
                if (!player.hurt(player.level().damageSources().generic(), 5F) && !level.isClientSide) return InteractionResultHolder.fail(originalStack)
            } else {
                player.hurt(player.level().damageSources().generic(), 5F)
...
class BloodBagItem(properties: Properties) : IVBagItem(properties) {
    override val bagUseSound = NTechSounds.emptyIVBag
