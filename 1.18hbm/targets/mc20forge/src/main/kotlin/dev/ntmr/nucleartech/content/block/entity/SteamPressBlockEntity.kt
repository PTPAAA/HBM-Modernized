...
class SteamPressBlockEntity(pos: BlockPos, state: BlockState) : RecipeMachineBlockEntity<PressingRecipe>(
    NTechBlockEntities.steamPressHeadBlockEntityType.get(), pos, state), RecipeHolder, ExperienceRecipeResultBlockEntity {
    var litDuration = 0
        private set
    var litTime = 0
        private set
    ...
    override fun getRenderBoundingBox() = AABB(blockPos.offset(0, -1, 0), blockPos.offset(1, 1, 1))
    ...
    override fun matchesRecipe(recipe: PressingRecipe) = !mainInventory[0].isEmpty && !mainInventory[1].isEmpty &&
        recipe.matches(this, getLevelUnchecked()) && canTransferItem(recipe.getResultItem(getLevelUnchecked().registryAccess()), mainInventory[3], this)
