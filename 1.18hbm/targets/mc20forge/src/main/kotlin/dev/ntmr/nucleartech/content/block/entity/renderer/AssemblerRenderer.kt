...
class AssemblerRenderer(@Suppress("UNUSED_PARAMETER") context: BlockEntityRendererProvider.Context) : BlockEntityRenderer<AssemblerBlockEntity> {
    override fun render(assembler: AssemblerBlockEntity, partials: Float, matrix: PoseStack, buffers: MultiBufferSource, light: Int, overlay: Int) {
        ...
        matrix.mulPose(com.mojang.math.Axis.YN.rotationDegrees(assembler.blockState.getValue(HorizontalDirectionalBlock.FACING).toYRot()))
        matrix.translate(-.5, .0, -.5)
        ...
        if (recipe != null) {
            ...
            if (stack.item is BlockItem) {
                ...
                matrix.mulPose(com.mojang.math.Axis.XP.rotationDegrees(-90F))
            }

            Minecraft.getInstance().itemRenderer.renderStatic(stack, net.minecraft.world.item.ItemDisplayContext.FIXED, light, overlay, matrix, buffers, assembler.level, assembler.blockState.getSeed(assembler.blockPos).toInt())
            matrix.popPose()
        }
        ...
        matrix.mulPose(com.mojang.math.Axis.ZP.rotationDegrees(-rotation))
