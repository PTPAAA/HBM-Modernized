...
class Cable(blockEntity: CableBlockEntity) : AbstractTransmitter<Cable, EnergyNetwork, IEnergyStorage>(blockEntity) {
    ...
    override fun isValidNetworkMember(blockEntity: BlockEntity, side: Direction) = blockEntity.getCapability(ForgeCapabilities.ENERGY, side.opposite).isPresent
    override fun getMember(blockEntity: BlockEntity, side: Direction) = blockEntity.getCapability(ForgeCapabilities.ENERGY, side.opposite).orElse(null)
