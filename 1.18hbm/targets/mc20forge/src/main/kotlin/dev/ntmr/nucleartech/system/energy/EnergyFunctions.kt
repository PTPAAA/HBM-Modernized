 ...
 * transferred. Both ItemStacks need to have the [Energy Capability][ForgeCapabilities.ENERGY].
 *
 * Negative values for [amount] are allowed, but may not always work and are unrecommended.
 */
@JvmOverloads
fun transferEnergy(from: ItemStack, to: ItemStack, amount: Int = Int.MAX_VALUE): Int =
    from.getCapability(ForgeCapabilities.ENERGY).map { fromStorage ->
        to.getCapability(ForgeCapabilities.ENERGY).map { toStorage ->
 ...
 * amount transferred. The [from] needs to have the [Energy Capability][ForgeCapabilities.ENERGY].
 *
 * Negative values for [amount] are allowed, but may not always work and are unrecommended.
 */
@JvmOverloads
fun transferEnergy(from: ItemStack, to: IEnergyStorage, amount: Int = Int.MAX_VALUE): Int =
    from.getCapability(ForgeCapabilities.ENERGY).map { itemStackEnergy ->
 ...
 * amount transferred. The [to] needs to have the [Energy Capability][ForgeCapabilities.ENERGY].
 *
 * Negative values for [amount] are allowed, but may not always work and are unrecommended.
 */
@JvmOverloads
fun transferEnergy(from: IEnergyStorage, to: ItemStack, amount: Int = Int.MAX_VALUE): Int =
    to.getCapability(ForgeCapabilities.ENERGY).map { itemStackEnergy ->
