...
import dev.ntmr.nucleartech.packets.ContaminationValuesUpdateMessage
...
object EntityContaminationEffects {
    fun update(entity: LivingEntity) {
        ...
        if (entity is ServerPlayer) {
            ...
            NuclearPacketHandler.sendTo(ContaminationValuesUpdateMessage(capability.serializeNBT()), entity as net.minecraft.server.level.ServerPlayer)
        }
    ...
    private fun handleRadiation(entity: LivingEntity, capability: ContaminationHandler) {
        ...
        when {
            entity is Creeper && irradiation >= 200 && !entity.isDeadOrDying -> if (world.random.nextInt(3) == 0) entity.convertTo(NTechEntities.nuclearCreeper.get(), true) else entity.hurt(DamageSources.radiation(world), 100F)
        ...
        if ((entity is Player && (entity.isCreative || entity.isSpectator)) || entity.isInvulnerable || entity.isInvulnerableTo(
                DamageSources.radiation(world)) || isRadiationImmune(entity)) return

        val random = world.random

        when {
            irradiation >= 1000 -> {
                entity.hurt(DamageSources.radiation(world), 3.4028235e38F)
