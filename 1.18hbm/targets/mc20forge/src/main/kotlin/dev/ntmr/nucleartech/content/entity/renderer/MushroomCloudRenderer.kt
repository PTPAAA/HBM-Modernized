...
import dev.ntmr.nucleartech.client.rendering.CameraShake
import dev.ntmr.nucleartech.client.rendering.NuclearRenderTypes
import dev.ntmr.nucleartech.client.rendering.SpecialModels
import dev.ntmr.nucleartech.client.rendering.renderModelAlpha
import dev.ntmr.nucleartech.content.entity.MushroomCloud
...
class MushroomCloudRenderer(context: EntityRendererProvider.Context) : EntityRenderer<MushroomCloud>(context) {
    ...
    private fun renderMush(
        ...
        modelRenderer.renderModelAlpha(matrix.last(), solidBuffer, null, model, red, green, blue, fade, light, OverlayTexture.NO_OVERLAY, ModelData.EMPTY)
        val texturedBuffer = renderer.getBuffer(NuclearRenderTypes.mushroomCloudTextured(getTextureLocation(entity), -progress * .035F))
        modelRenderer.renderModelAlpha(matrix.last(), texturedBuffer, null, model, 1F, 1F, 1F, fade * .875F, light, OverlayTexture.NO_OVERLAY, ModelData.EMPTY)
    ...
    private fun renderFlash(entity: MushroomCloud, matrix: PoseStack, partialTicks: Float, renderer: MultiBufferSource) {
        ...
        for (i in 0 until 300) {
            matrix.mulPose(Axis.XP.rotationDegrees(random.nextFloat() * 360F))
            matrix.mulPose(Axis.YP.rotationDegrees(random.nextFloat() * 360F))
            matrix.mulPose(Axis.ZP.rotationDegrees(random.nextFloat() * 360F))
            matrix.mulPose(Axis.XP.rotationDegrees(random.nextFloat() * 360F))
            matrix.mulPose(Axis.YP.rotationDegrees(random.nextFloat() * 360F))
            matrix.mulPose(Axis.ZP.rotationDegrees(random.nextFloat() * 360F))
    ...
    private fun shakeCamera(entity: MushroomCloud) { // TODO shake intensity depending on distance
        ...
        if (distanceToShockwave > -5 && distanceToShockwave < 5)
            CameraShake.setIntensityAtLeast(1.0) // TODO calculate shockwave strength?
        val distanceForGroundShake = distance - entity.age * 5
        if (distanceForGroundShake < 10) {
            if (distanceForGroundShake < 0) CameraShake.setIntensityAtLeast(entity.scale * 0.2)
            else CameraShake.setIntensityAtLeast(entity.scale * 0.7)
        }
    }
}
