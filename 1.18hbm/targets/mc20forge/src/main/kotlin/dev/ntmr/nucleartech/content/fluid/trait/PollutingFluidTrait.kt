        ...
        if (flag.isAdvanced) {
            ...
            if (!spillData.isZeroed) {
                ...
                for ((type, amount) in spillData.toMap()) {
                    if (amount == 0F)
                        continue
                    tooltip += Component.literal(" - ").append(typeToTranslationKey(type).format(String.format(java.util.Locale.ROOT, "%.2f", amount * 1000F))).green()
                }
            }
            ...
            if (!burnData.isZeroed) {
                ...
                for ((type, amount) in burnData.toMap()) {
                    if (amount == 0F)
                        continue
                    tooltip += Component.literal(" - ").append(typeToTranslationKey(type).format(String.format(java.util.Locale.ROOT, "%.2f", amount * 1000F))).red()
                }
            }
        }
    }
