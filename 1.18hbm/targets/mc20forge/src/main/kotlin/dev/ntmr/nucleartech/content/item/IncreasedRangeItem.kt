    ...
    companion object {
        ...
        fun addItemStackAttributes(event: ItemAttributeModifierEvent) {
            ...
            if (item.additionalRange > 0F && event.slotType == EquipmentSlot.MAINHAND) {
                event.addModifier(ForgeMod.BLOCK_REACH.get(), AttributeModifier(INCREASED_RANGE_UUID, "Increased range multi-block placement", item.additionalRange.toDouble(), AttributeModifier.Operation.ADDITION))
            }
        }
