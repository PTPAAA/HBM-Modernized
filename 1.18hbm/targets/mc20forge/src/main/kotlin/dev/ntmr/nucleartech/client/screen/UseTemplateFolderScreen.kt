...
import dev.ntmr.nucleartech.getTagManager
import dev.ntmr.nucleartech.packets.CraftMachineTemplateMessage
...
class UseTemplateFolderScreen : Screen(NTechItems.machineTemplateFolder.get().description) {
    ...
    private fun craftRecipe(index: Int) {
        ...
        NuclearPacketHandler.sendToServer(CraftMachineTemplateMessage(
    ...
    override fun render(graphics: GuiGraphics, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        graphics.blit(TEMPLATE_FOLDER_GUI_LOCATION, x, y, 0, 0, GUI_WIDTH, GUI_HEIGHT)
        ...
        // page number
        graphics.drawCenteredString(font, Component.literal("$currentPage / $pagesCount"), width / 2, (height - GUI_HEIGHT) / 2 + 10, 0xFFFFFF)
        ...
        if (searchBox.isFocused) {
            ...
            RenderSystem.setShaderColor(1f, 1f, 1f, 1f)
            RenderSystem.setShaderTexture(0, TEMPLATE_FOLDER_GUI_LOCATION)
            graphics.blit(TEMPLATE_FOLDER_GUI_LOCATION, x + 45, y + 211, 176, 54, 72, 12)
        }
        ...
        for (i in templateButtons.indices) {
            ...
            if (button.isHoveredOrFocused && button.visible) {
                ...
                graphics.renderTooltip(font, getTooltipFromItem(minecraft!!, itemStack), java.util.Optional.empty(), mouseX, mouseY)
            }
        }
    }

    private fun renderButtonItem(item: ItemStack, x: Int, y: Int, graphics: GuiGraphics) {
        graphics.pose().pushPose(); graphics.pose().translate(0.0f, 0.0f, 100.0f)
    ...
    private fun refreshSearchResults() {
        ...
        val lowerString = searchString.lowercase(java.util.Locale.ROOT)
        searchResults.addAll(itemList.filter { 
             getTooltipFromItem(minecraft!!, it).any { component -> 
                 component.string.lowercase(java.util.Locale.ROOT).contains(lowerString)
             }
        })
        currentPage = 1
    ...
    class ChangePageButton(x: Int, y: Int, val isForward: Boolean, onPress: OnPress) :
        Button(Button.Builder(Component.empty(), onPress).bounds(x, y, 18, 18)) {
        ...
        override fun renderWidget(graphics: GuiGraphics, mouseX: Int, mouseY: Int, partialTicks: Float) {
            ...
            graphics.blit(TEMPLATE_FOLDER_GUI_LOCATION, this.x, this.y, x, y, 18, 18)
        }
    }

    override fun isPauseScreen(): Boolean = false

    inner class CraftButton(x: Int, y: Int, val index: Int) : Button(Button.Builder(Component.empty()) { craftRecipe(index) }.bounds(x, y, 18, 18)) {
        override fun renderWidget(graphics: GuiGraphics, mouseX: Int, mouseY: Int, partialTicks: Float) {
            RenderSystem.setShader(GameRenderer::getPositionTexShader)
            RenderSystem.setShaderColor(1f, 1f, 1f, 1f)
            RenderSystem.setShaderTexture(0, TEMPLATE_FOLDER_GUI_LOCATION)
            ...
            graphics.blit(TEMPLATE_FOLDER_GUI_LOCATION, this.x, this.y, x, 0, 18, 18)
        }
    }
    ...
    companion object {
        ...
        fun getAllItems(): List<ItemStack> = (ForgeRegistries.ITEMS.tags()?.getTag(NTechTags.Items.FOLDER_STAMPS)?.stream()?.map(::ItemStack)?.toList() ?: emptyList()) +
            (ForgeRegistries.ITEMS.tags()?.getTag(NTechTags.Items.SIREN_TRACKS)?.stream()?.map(::ItemStack)?.toList() ?: emptyList()) +
        ...
        fun reloadSearchTree() {
            ...
            NuclearTech.LOGGER.debug("Reloading Machine Template Folder search tree (No-op in 1.20)")
        }
    }
}
