...
import dev.ntmr.nucleartech.api.fluid.trait.getTraitForFluidStack
...
class TurbineBlockEntity(pos: BlockPos, state: BlockState) : BaseMachineBlockEntity(NTechBlockEntities.turbineBlockEntityType.get(), pos, state), TickingServerBlockEntity {
    ...
    override fun isItemValid(slot: Int, stack: ItemStack): Boolean = when (slot) {
        ...
        1 -> true
        2 -> stack.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM).isPresent
        3 -> true
        4 -> stack.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM).isPresent
        5 -> false
        6 -> stack.getCapability(ForgeCapabilities.ENERGY).isPresent
        else -> false
    }
    ...
    override fun saveAdditional(tag: CompoundTag) {
        ...
        tag.put("InputTank", inputTank.writeToNBT(CompoundTag()).apply { putString("FluidName", net.minecraftforge.registries.ForgeRegistries.FLUIDS.getKey(forcedType)!!.toString()) })
    ...
    private val fluidCapabilityDelegate = object : ForceableFluidHandler {
    ...
    override fun registerCapabilityHandlers() {
        ...
        registerCapabilityHandler(ForgeCapabilities.FLUID_HANDLER, this::fluidCapabilityDelegate)
    }

    companion object {
        const val MAX_ENERGY = 1_000_000
        const val MAX_INPUT = FluidType.BUCKET_VOLUME * 64
        const val MAX_OUTPUT = FluidType.BUCKET_VOLUME * 128
    }
}
