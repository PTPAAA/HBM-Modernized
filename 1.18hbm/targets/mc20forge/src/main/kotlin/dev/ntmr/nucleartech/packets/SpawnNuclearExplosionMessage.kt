        ...
        if (context.get().direction.receptionSide.isServer)
            context.get().enqueueWork {
                ...
                if (!sender.canUseGameMasterBlocks()) {
                    sender.sendSystemMessage(UseCreativeNuclearExplosionSpawnerScreen.ERR_INSUFFICIENT_PERMISSION)
                    return@enqueueWork
                }
                NukeExplosion.createAndStart(sender.level(), position, strength.toFloat(), NuclearExplosionMk4Params(hasFallout, extraFallout, muted))
            }
