...
import dev.ntmr.nucleartech.api.explosion.Explosion
import dev.ntmr.nucleartech.api.explosion.ExplosionFactory
...
class NukeExplosion(entityType: EntityType<NukeExplosion>, world: Level) : Entity(entityType, world) {
    ...
    override fun remove(reason: RemovalReason) {
        ...
        if (hasFallout) {
            val fallout = FalloutRain(NTechEntities.falloutRain.get(), level())
    ...
    companion object : ExplosionFactory<NuclearExplosionMk4Params> {
        fun dealDamage(world: Level, pos: Vec3, radius: Double, maxDamage: Float = 250F) {
            ...
            for (entity in entities) {
                ...
                if (distance <= radius && world.clip(ClipContext(pos, entity.eyePosition, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, null)).type == HitResult.Type.MISS) {
                    ...
                    entity.hurt(DamageSources.nuclearBlast(world), damage.toFloat())
        ...
        override fun create(level: Level, pos: Vec3, strength: Float, params: NuclearExplosionMk4Params): Explosion? = if (level.isClientSide) null
            ...
            val explosionEntity = NukeExplosion(NTechEntities.nuclearExplosion.get(), level).apply {
