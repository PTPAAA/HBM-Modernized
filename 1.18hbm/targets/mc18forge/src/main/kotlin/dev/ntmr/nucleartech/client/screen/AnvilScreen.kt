...
class AnvilScreen(anvilMenu: AnvilMenu, playerInventory: Inventory, title: Component) : AbstractContainerScreen<AnvilMenu>(anvilMenu, playerInventory, title) {
    ...
    private val originalRecipeList = menu.playerInventory.player.level.recipeManager.getAllRecipesFor(NTechRecipeTypes.CONSTRUCTING).filter { it.isTierWithinBounds(tier) }.sortedBy { it.resultItem.displayName.string }
    ...
    companion object {
        ...
        fun reloadSearchTree() {
            ...
            if (level != null) {
                level.recipeManager.getAllRecipesFor(NTechRecipeTypes.CONSTRUCTING).forEach(anvilConstructingRecipeSearchTree::add)
