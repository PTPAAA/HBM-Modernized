...
import dev.ntmr.nucleartech.content.recipe.*
...
import dev.ntmr.nucleartech.integration.jei.categories.*
import dev.ntmr.nucleartech.integration.jei.categories.TemplateFolderJRC.TemplateFolderRecipe
import dev.ntmr.nucleartech.integration.jei.categories.TemplateFolderJRC.TemplateFolderRecipe.TemplateType
import dev.ntmr.nucleartech.integration.jei.transfers.PressingJRTI
...
class JeiIntegration : IModPlugin {
    ...
    override fun getPluginUid() = ntm(MODID)
    ...
    override fun registerRecipes(registration: IRecipeRegistration) {
        ...
        val recipeMap = recipeManager.recipes
            .filter { it.id.namespace == MODID }
            .groupBy { it.type }
            .withDefault { emptyList() }
        registration.addRecipes(NuclearRecipeTypes.ASSEMBLING, recipeMap.getValue(NTechRecipeTypes.ASSEMBLY).filterIsInstance<AssemblyRecipe>())
        registration.addRecipes(NuclearRecipeTypes.BLASTING, recipeMap.getValue(NTechRecipeTypes.BLASTING).filterIsInstance<BlastingRecipe>())
        registration.addRecipes(NuclearRecipeTypes.CHEMISTRY, recipeMap.getValue(NTechRecipeTypes.CHEM).filterIsInstance<ChemRecipe>())
        registration.addRecipes(NuclearRecipeTypes.CONSTRUCTING, recipeMap.getValue(NTechRecipeTypes.CONSTRUCTING).filterIsInstance<AnvilConstructingRecipe>())
        registration.addRecipes(NuclearRecipeTypes.PRESSING, recipeMap.getValue(NTechRecipeTypes.PRESSING).filterIsInstance<PressingRecipe>())
        registration.addRecipes(NuclearRecipeTypes.SHREDDING, recipeMap.getValue(NTechRecipeTypes.SHREDDING).filterIsInstance<ShreddingRecipe>())
        registration.addRecipes(NuclearRecipeTypes.SMITHING, recipeMap.getValue(NTechRecipeTypes.SMITHING).filterIsInstance<AnvilSmithingRecipe>())
    }
