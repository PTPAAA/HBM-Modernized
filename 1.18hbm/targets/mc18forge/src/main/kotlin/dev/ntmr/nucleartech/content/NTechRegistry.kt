...
interface NTechRegistry<V : IForgeRegistryEntry<V>> {
    ...
    fun <R : V> registerBefore(target: String, name: String, sup: () -> R): RegistryObject<R> {
        addHook(RegistryHook.Before<V>(forgeRegistry.registryName!!, target) { callback ->
            callback.register(name, sup)
        })
        return RegistryObject.create(ResourceLocation(MODID, name), forgeRegistry.registryName!!, MODID)
    }

    fun <R : V> registerAfter(target: String, name: String, sup: () -> R): RegistryObject<R> {
        addHook(RegistryHook.After<V>(forgeRegistry.registryName!!, target) { callback, _ ->
            callback.register(name, sup)
        })
        return RegistryObject.create(ResourceLocation(MODID, name), forgeRegistry.registryName!!, MODID)
    }
