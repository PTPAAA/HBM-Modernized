...
object TheFogIsComing {
    ...
    fun theFogHasArrived(event: EntityViewRenderEvent.RenderFogEvent) {
    ...
    @JvmStatic @SubscribeEvent
    fun theFogTheFogTheFogTheFog(event: EntityViewRenderEvent.FogColors) {
        val soot = (renderSoot - NuclearConfig.client.sootFogThreshold.get()).toFloat()
        val sootColor = 0.15F
    ...
    fun theFogWillReturn(event: WorldEvent.Unload) {
        targetSoot = 0F
        renderSoot = -1F
    }
}
