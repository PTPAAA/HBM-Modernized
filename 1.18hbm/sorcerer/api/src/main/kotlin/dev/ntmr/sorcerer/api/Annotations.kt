...
annotation class Linkage(val mcVersion: String, val to: String) {
     ...
     *
     * Stubs will be generated at `dev.ntmr.sorcerer.generated.<src-set-name>.ApiStubs` and will be extensions
...
@Retention(SOURCE)
@MustBeDocumented
annotation class MirrorBlueprint {
    @Target(CLASS)
    @Retention(SOURCE)
    @Repeatable
    @MustBeDocumented
    annotation class Constructor(val mcVersion: String, val functionNotation: String, val mode: ConstructorMode = ConstructorMode.AUTO) {
        enum class ConstructorMode { PRIMARY, SECONDARY, ALT, AUTO }
    }

    @Target(CLASS)
    @Retention(SOURCE)
    @MustBeDocumented
    annotation class UnreachableConstructor(val mcVersions: Array<String> = [])
