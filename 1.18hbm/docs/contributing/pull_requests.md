...
Besides having a Codeberg account, this document expects you to use the [Git command line interface](https://git-scm.com/), which you'll have to install if it isn't already, and set up when running for the first time. While GUI clients do exist, the following instructions contain commands for the CLI.
...
In order for your changes to be merged, you need to upload them to Codeberg. But you don't have permission to push commits into the official repository, which is why you'll have to [create a fork](https://codeberg.org/MartinTheDragon/Nuclear-Tech-Mod-Remake/fork).
...
```bash
git remote set-url origin https://codeberg.org/YOUR-USERNAME/Nuclear-Tech-Mod-Remake.git
```
...
If everything worked this far, great! You successfully uploaded your changes to Codeberg, where you can create a pull request.

### Opening a pull request

Codeberg already has [pretty good documentation](https://docs.codeberg.org/collaborating/pull-requests-and-git-flow/) on how to do that, so follow that.
