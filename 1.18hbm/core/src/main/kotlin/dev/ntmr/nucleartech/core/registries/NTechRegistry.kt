...
package dev.ntmr.nucleartech.core.registries
...
import dev.ntmr.sorcerer.generated.spells.main.net.minecraftforge.registries.IForgeRegistryEntry
import dev.ntmr.sorcerer.generated.spells.main.net.minecraftforge.registries.RegistryObject

interface NTechRegistry<V : IForgeRegistryEntry<V>> {
    ...
    fun <R : V> registerBefore(target: String, name: String, sup: () -> R): RegistryObject<R> {
        addHook(RegistryHook.Before<V>(forgeRegistry.getRegistryName()!!, target) { callback ->
            callback.register(name, sup)
        })
        return RegistryObject.create(ResourceLocation(MODID, name), forgeRegistry.getRegistryName()!!, MODID)
    }

    fun <R : V> registerAfter(target: String, name: String, sup: () -> R): RegistryObject<R> {
        addHook(RegistryHook.After<V>(forgeRegistry.getRegistryName()!!, target) { callback, _ ->
            callback.register(name, sup)
        })
        return RegistryObject.create(ResourceLocation(MODID, name), forgeRegistry.getRegistryName()!!, MODID)
    }
